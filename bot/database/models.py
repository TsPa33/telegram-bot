...
async def create_tables():
    ...
    await execute("CREATE INDEX IF NOT EXISTS idx_lead_thread_messages_lead_created ON lead_thread_messages(lead_id, created_at, id);")
    await execute("CREATE INDEX IF NOT EXISTS idx_lead_thread_messages_proposal_created ON lead_thread_messages(proposal_id, created_at, id);")
    await execute("CREATE INDEX IF NOT EXISTS idx_lead_thread_messages_telegram_reply ON lead_thread_messages(telegram_chat_id, telegram_message_id);")
    ...
    await execute("ALTER TABLE sellers ADD COLUMN IF NOT EXISTS crm_enabled BOOLEAN NOT NULL DEFAULT FALSE;")
    ...
    await execute("CREATE INDEX IF NOT EXISTS idx_seller_products_seller_id ON seller_products(seller_id);")
    await execute("CREATE INDEX IF NOT EXISTS idx_seller_products_donor_car_id ON seller_products(donor_car_id);")
    await execute("CREATE INDEX IF NOT EXISTS idx_seller_products_category ON seller_products(category);")
    await execute("CREATE INDEX IF NOT EXISTS idx_seller_products_status ON seller_products(status);")
    await execute("CREATE INDEX IF NOT EXISTS idx_seller_products_title ON seller_products(title);")
    await execute("CREATE INDEX IF NOT EXISTS idx_seller_products_oem_code ON seller_products(oem_code);")
    ...
    CREATE TABLE IF NOT EXISTS buyer_favorites (
        ...
        entity_type TEXT NOT NULL
            CHECK (entity_type IN ('car', 'seller', 'service', 'website')),
    ...
    CREATE TABLE IF NOT EXISTS buyer_requests (
        ...
        status TEXT NOT NULL DEFAULT 'new'
            CHECK (status IN ('new', 'viewed', 'answered', 'closed')),
        message TEXT,
        created_at TIMESTAMP NOT NULL DEFAULT NOW(),
        updated_at TIMESTAMP NOT NULL DEFAULT NOW()
    );
    ...
    CREATE TABLE IF NOT EXISTS buyer_history (
        ...
        telegram_id BIGINT NOT NULL,
        entity_type TEXT NOT NULL
            CHECK (entity_type IN ('car', 'seller', 'service')),
        entity_ref TEXT NOT NULL,
    ...
    await execute("CREATE INDEX IF NOT EXISTS idx_buyer_request_offers_status_request ON buyer_request_offers(request_id, status, updated_at DESC);")
    ...
    """)
    await execute("CREATE INDEX IF NOT EXISTS idx_marketplace_matches_seller_status ON marketplace_matches(seller_id, status, matched_at DESC);")
    await execute("CREATE INDEX IF NOT EXISTS idx_marketplace_matches_offer ON marketplace_matches(offer_id);")
